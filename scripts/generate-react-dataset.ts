import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { getDb } from '../src/db/index.js';
import { GoogleGenAI } from '@google/genai';
import { env } from '../src/config/env.js';
import { logger } from '../src/utils/logger.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const apiKeys = [env.GEMINI_API_KEY, env.GEMINI_API_KEY_2, env.GEMINI_API_KEY_3].filter(Boolean) as string[];
if (apiKeys.length === 0) {
  logger.error('No GEMINI_API_KEY provided in environment variables.');
  process.exit(1);
}

let currentKeyIndex = 0;
let ai = new GoogleGenAI({ apiKey: apiKeys[currentKeyIndex] });

function rotateApiKey() {
  currentKeyIndex = (currentKeyIndex + 1) % apiKeys.length;
  logger.info(`Switching to API key index ${currentKeyIndex}`);
  ai = new GoogleGenAI({ apiKey: apiKeys[currentKeyIndex] });
}

// ============================================================
// React + Next.js Curriculum — 70 Topics across 6 Modules
// ============================================================
const modules = [
  {
    name: '1. React Fundamentals',
    topics: [
      'What is React?',
      'Why React?',
      'Project Structure',
      'JSX Syntax',
      'Functional Components',
      'Props',
      'Children Prop',
      'State with useState',
      'Event Handling',
      'Conditional Rendering',
      'Lists & Keys',
      'Controlled vs Uncontrolled Components',
      'Forms in React',
      'Component Composition',
      'Lifting State Up',
      'React DevTools',
    ],
  },

  {
    name: '2. React Hooks',
    topics: [
      'Rules of Hooks',
      'useEffect',
      'Dependency Array',
      'Cleanup Function',
      'useRef',
      'useContext',
      'useReducer',
      'Custom Hooks',
      'useMemo',
      'useCallback',
      'useId',
      'useTransition',
      'useDeferredValue',
    ],
  },

  {
    name: '3. React Under the Hood',
    topics: [
      'Virtual DOM',
      'Reconciliation',
      'Rendering Process',
      'Component Lifecycle',
      'React Strict Mode',
      'Batching Updates',
      'React.memo',
      'Lazy Loading',
      'Suspense',
      'Code Splitting',
      'Error Boundaries',
      'Portals',
      'Forward Ref',
      'useImperativeHandle',
      'useLayoutEffect',
    ],
  },

  {
    name: '4. Advanced React Patterns',
    topics: [
      'Compound Components',
      'Higher Order Components (HOC)',
      'Render Props',
      'Context Patterns',
      'Performance Optimization',
      'Memoization Strategy',
      'Folder Structure',
      'Feature Based Architecture',
      'Reusable Components',
      'Accessibility (a11y)',
    ],
  },

  {
    name: '5. Next.js App Router',
    topics: [
      'What is Next.js?',
      'Why Next.js?',
      'Project Structure',
      'App Router',
      'File Based Routing',
      'Dynamic Routes',
      'Nested Routes',
      'Route Groups',
      'Layouts',
      'Templates',
      'Loading UI',
      'Error UI',
      'Not Found Page',
      'Metadata API',
      'Fonts',
      'Image Optimization',
      'Link Component',
      'Navigation',
    ],
  },

  {
    name: '6. Rendering in Next.js',
    topics: [
      'Client Components',
      'Server Components',
      'SSR',
      'SSG',
      'ISR',
      'Streaming',
      'Hydration',
      'Hydration Errors',
      'Caching',
      'Revalidation',
      'Dynamic Rendering',
      'Server Actions',
    ],
  },

  {
    name: '7. APIs & Data Fetching',
    topics: [
      'Fetching Data',
      'Async Server Components',
      'API Routes',
      'Route Handlers',
      'Cookies',
      'Headers',
      'Search Params',
      'Axios Setup',
      'Fetch vs Axios',
      'Environment Variables',
    ],
  },

  {
    name: '8. State Management',
    topics: [
      'Local State',
      'Prop Drilling',
      'Context API',
      'Redux Toolkit',
      'Redux Toolkit Query',
      'Zustand',
      'Jotai',
      'TanStack Query',
      'SWR',
      'Choosing the Right State Management',
    ],
  },

  {
    name: '9. Authentication & Security',
    topics: [
      'Authentication Basics',
      'JWT Authentication',
      'Session Authentication',
      'NextAuth',
      'Protected Routes',
      'Role Based Access',
      'Refresh Tokens',
      'Secure Cookies',
      'Middleware Authentication',
    ],
  },

  {
    name: '10. Real World React',
    topics: [
      'React Hook Form',
      'Zod Validation',
      'Debounced Search',
      'Infinite Scroll',
      'Pagination',
      'File Upload',
      'Drag & Drop',
      'Toast Notifications',
      'Modal Patterns',
      'Optimistic UI',
      'Loading Skeleton',
      'Error UI',
      'Dark Mode',
      'Theme Switching',
    ],
  },

  {
    name: '11. Performance Optimization',
    topics: [
      'When to use useMemo',
      'When to use useCallback',
      'React.memo',
      'Bundle Optimization',
      'Code Splitting',
      'Lazy Loading',
      'Image Optimization',
      'Avoiding Re-renders',
      'Profiler',
      'Performance Checklist',
    ],
  },

  {
    name: '12. Deployment & Production',
    topics: [
      'Production Build',
      'Environment Variables',
      'Vercel Deployment',
      'Netlify Deployment',
      'SEO Basics',
      'Open Graph',
      'Robots.txt',
      'Sitemap',
      'Analytics',
      'Production Best Practices',
    ],
  },
];
// Build a flat ordered list for next_topic lookup
const allTopics = modules.flatMap(m => m.topics);

async function generateTopicContent(
  moduleName: string,
  topic: string,
  nextTopic: string,
  retries = 0
): Promise<any> {
  const prompt = `
You are a world-class React and Next.js educator creating premium Instagram learning content.

Topic: ${topic}
Module: ${moduleName}
Next Topic (for CTA): ${nextTopic}

Generate a JSON object with EXACTLY these fields:

{
  "hook_text": string,
  "explanation_1": array,
  "code": object,
  "explanation_2": object,
  "real_world_usecase": string,
  "common_edge_cases": string,
  "interview_question": string,
  "pro_tip": string,
  "difficulty": string
}

REQUIREMENTS:

1. hook_text
   - ONE sentence. Max 15 words.
   - Explain WHY this topic matters in plain English.
   - Use **word** syntax to bold 1-2 key terms.
   - Example: "Stop **expensive calculations** from running on every render."

2. explanation_1 (WHAT & WHY bullets)
   - Return a JSON ARRAY of exactly 4 strings.
   - Each string = one bullet. Max 10 words per bullet.
   - Use **word** for emphasis. No HTML tags.
   - Example: ["Stores a **calculated value** in memory", "Prevents **unnecessary recalculations**", "Improves performance on **re-renders**", "Best for **expensive operations**"]

3. code (Before/After comparison)
   - Return a JSON OBJECT with 4 fields:
     - "before_label": short label like "Without useMemo"
     - "before": max 4 lines of code showing the WRONG/naive approach
     - "after_label": short label like "With useMemo"
     - "after": max 6 lines of code showing the CORRECT approach
   - Use realistic variable names. Highlight the key improvement.
   - NO comments unless truly necessary. Code should be self-explanatory.

4. explanation_2 (How it Works — Timeline)
   - Return a JSON OBJECT with one field:
     - "steps": array of 3-4 short strings describing what happens step-by-step
     - Each step: max 12 words. Plain English. No bullet points inside strings.
   - Example: {"steps": ["Component renders — useMemo always runs first time", "React checks the dependency array [items]", "Changed? Yes → Recalculate. No → Return cached value."]}
   - Last step CAN include a Yes/No branch like: "Changed? Yes → recalc. No → cached."

5. real_world_usecase
   - 2-3 SHORT sentences. Max 40 words total.
   - Describe a real production scenario where this concept saves time or prevents bugs.
   - Keep sentences short. Use **word** for emphasis.

6. common_edge_cases (Gotcha)
   - 2-3 SHORT sentences. Max 40 words total.
   - Describe the #1 mistake developers make with this concept.
   - Start with a clear warning. Use **word** for emphasis.

7. interview_question
   - ONE question (string). Should test deep understanding, not just syntax.
   - Format: "When should you NOT use ${topic}?" or similar probing question.
   - Max 20 words.

8. pro_tip
   - ONE crisp sentence. Max 20 words. A professional best practice.
   - Use **word** for emphasis.

9. difficulty
   - Must be exactly one of: "Beginner", "Intermediate", "Advanced"

GOLDEN RULES:
- Every field must be beginner-friendly. No jargon without explanation.
- Short sentences win over long sentences.
- Code must be realistic — show actual patterns developers use daily.
- Avoid generic advice. Give specific, actionable insights.

Respond ONLY with valid raw JSON.
Do NOT include markdown, code fences, comments, or extra text.
`;

  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: prompt,
    });

    let text = response.text?.trim() || '{}';
    if (text.startsWith('```json')) {
      text = text.replace(/^```json\n/, '').replace(/\n```$/, '');
    } else if (text.startsWith('```')) {
      text = text.replace(/^```\n/, '').replace(/\n```$/, '');
    }

    const parsed = JSON.parse(text);
    return parsed;
  } catch (err: any) {
    logger.error(`Failed to generate content for "${topic}" with key ${currentKeyIndex}:`, err.message);
    if (retries < apiKeys.length - 1) {
      rotateApiKey();
      return generateTopicContent(moduleName, topic, nextTopic, retries + 1);
    }
    return null;
  }
}

async function seedDatabase() {
  const db = await getDb();
  let day = 1;

  for (const mod of modules) {
    for (const topic of mod.topics) {
      const topicIndex = allTopics.indexOf(topic);
      const nextTopic = allTopics[topicIndex + 1] || 'useCallback';

      logger.info(`Generating React Mastery Day ${day}: ${topic}...`);

      const exists = await db.get(
        'SELECT id FROM posts WHERE series = ? AND day = ?',
        ['react-mastery', day]
      );
      if (exists) {
        logger.info(`Day ${day} already exists, skipping.`);
        day++;
        continue;
      }

      const content = await generateTopicContent(mod.name, topic, nextTopic);

      if (content) {
        // Serialize complex fields as JSON strings
        const codeStr = typeof content.code === 'object'
          ? JSON.stringify(content.code)
          : content.code || '{}';

        const e1Str = Array.isArray(content.explanation_1)
          ? JSON.stringify(content.explanation_1)
          : content.explanation_1 || '[]';

        const e2Str = typeof content.explanation_2 === 'object'
          ? JSON.stringify(content.explanation_2)
          : content.explanation_2 || '{}';

        await db.run(
          `INSERT INTO posts
          (series, day, title, difficulty, code, question, answer, explanation, hook_text,
           explanation_1, explanation_2, pro_tip, module_name, real_world_usecase,
           common_edge_cases, interview_question)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            'react-mastery',
            day,
            topic,
            content.difficulty || 'Intermediate',
            codeStr,
            nextTopic,           // question field stores next_topic for CTA
            '',                  // answer unused
            '',                  // explanation unused
            content.hook_text || '',
            e1Str,
            e2Str,
            content.pro_tip || '',
            mod.name,
            content.real_world_usecase || '',
            content.common_edge_cases || '',
            content.interview_question || '',
          ]
        );
        logger.info(`✅ Saved React Mastery Day ${day}: ${topic}`);
      } else {
        logger.warn(`⚠️  Skipped Day ${day}: ${topic} (generation failed)`);
      }

      day++;
      // Respect rate limits
      await new Promise(r => setTimeout(r, 2500));
    }
  }

  logger.info('✅ React Mastery dataset generation complete!');
}

seedDatabase().catch(err => logger.error(err));
